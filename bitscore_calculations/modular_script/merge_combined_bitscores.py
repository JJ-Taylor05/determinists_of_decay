#!/usr/bin/env python3
"""
merge_combined_bitscores.py
===============
Merges combined FIMO sum-of-bits scores with mRNA degradation data by mapping
transcript IDs (ENST) to gene IDs (ENSG) via the Ensembl REST API. 

Note that this script is updated from the original merge_combined_bitscores.py used 
in the quantile method to account for differences in the outputs of the modular script.

If you run into any issues with Ensembl; its not you, its them. The likely solution is
to adjust the BATCH_SIZE and/or RETRY_BACKOFF constants in this script. You can also try
increasing the RETRY_LIMIT and time.sleep() between batches. Godspeed.
...
Output columns
--------------
gene_id | gene_name | transcript_id | degradation_value | sum_of_bits_combined_score

Usage
-----
    python merge_scores.py \\
        --bits   sum_of_bits_combined_scores.tsv \\
...
def parse_args():
    ...
    p.add_argument("--bits", required=True,
                   help="Sum-of-bits TSV (sequence_id, n_motifs_hit, sum_of_bits_combined_score)")
...
def load_bits(path):
    """
    Returns dict: transcript_id (with version) -> sum_of_bits_combined_score
    ...
    with open(path) as fh:
        ...
        for row in reader:
            bits[row["sequence_id"]] = float(row["sum_of_bits_combined_score"])
...
def merge(bits, halflife, transcript_to_gene):
    ...
    for transcript_id, bit_score in bits.items():
        ...
        rows.append({
            ...
            "sum_of_bits_combined_score": bit_score,
        })
...
def write_output(path, rows):
    fieldnames = ["gene_id", "gene_name", "transcript_id",
                  "degradation_value", "sum_of_bits_combined_score"]
